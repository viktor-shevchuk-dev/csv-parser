...
api.get('/swagger.yaml', (ctx, _next) => {
  ctx.status = 200
  ctx.body = swaggerSpec.toString()
})

api.get('/movies', async (ctx, _next) => {
...
api.get('/movies/:id', async (ctx, _next) => {
...
api.delete('/movies/:id', async (ctx, _next) => {
  const id = parseInt(ctx.params.id, 10)
...
api.post('/movies', async (ctx, _next) => {
...
api.put('/movies', async (ctx, _next) => {
  console.log('Request body: ', ctx.request.body)
